...
defmodule AutogluonMcp.HttpServer do
  @moduledoc """
  HTTP server for AutoGluon MCP using ExMCP.HttpPlug.
  ...
  def start_link(opts) do
    ...
    Logger.info("Starting AutoGluon MCP HTTP server on #{host}:#{port}")

    # Use Router which adds health check endpoint and forwards to ExMCP.HttpPlug
    case Plug.Cowboy.http(AutogluonMcp.Router, [], port: port, ip: parse_host(host)) do
      {:ok, pid} ->
        Logger.info("AutoGluon MCP HTTP server started successfully on port #{port}")
        {:ok, pid}

      {:error, {:already_started, pid}} ->
        Logger.info("AutoGluon MCP HTTP server already running")
        {:ok, pid}

      {:error, reason} ->
        Logger.error("Failed to start AutoGluon MCP HTTP server: #{inspect(reason)}")
        {:error, reason}
    end
  end
